...
Page({
  ...
  onLoad: function (options) {
    ...
    util.readRecordlist('OfficeCharge', options.id, this, '办公费用')
    if (options.id) {
      detailofficeCost({
        ID: options.id
      }).then(res => {
        ...
        if (res.code == 10000) {
          ...
          util.getbutton(item.ID, 'OfficeCharge', item.CurStepbh, this);
          ...
          util.workList(this, mid, 'OfficeCharge', options.id)
  ...
  change12(e) {
    ...
    if (e.currentTarget.dataset.i) {
      ...
      wx.redirectTo({
        url: '/OAmoudle/pages/officeCost/detail/detail?history=5&id=' + JSON.parse(e.currentTarget.dataset.i).ID
      })
    }
  },
  // 工作流流转
  ...
  tconfirm() {
    ...
    if (util.Reclick(dangqiantime)) {
      if (this.data.ApprovalOpinion) {
        util.Triggerflow(this, 'return', 'OfficeCharge', 'officeCost', '', '', '', '', '', '', 'oa', this.data.ApprovalOpinion ? this.data.ApprovalOpinion : '不同意。', JSON.stringify(this.data.idea.API_Picurl), JSON.stringify(this.data.idea.API_Fileurl))
      } else {
        wx.showToast({
          title: '请输入审批意见',
          icon: 'none',
  ...
  sconfirm() {
    var dangqiantime = (Date.parse(new Date()) / 1000);
    if (util.Reclick(dangqiantime)) {
      util.Triggerflow(this, 'next', 'OfficeCharge', 'officeCost', '', '', '', '', '', '', 'oa', this.data.ApprovalOpinion ? this.data.ApprovalOpinion : '同意。', JSON.stringify(this.data.idea.API_Picurl), JSON.stringify(this.data.idea.API_Fileurl))
    }
  },
  // 审核通过
  putin() {
