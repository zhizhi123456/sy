...
Page({
  ...
  seachInfo() {
    ...
    getExpressSent({
      ...
      if (res.code == 10000) {
        item = res.List;
        list = util.listData(item.reverse(), app.globalData.department, this.data.pages, list, this, 'ExpressSent');
        this.setData({
          InfoList: list,
          item,
          seach: ''
  ...
  onConfirm_seach() {
    ...
    if (this.data.info.Expressname || this.data.info.classID || this.data.info.ExpressCompany || this.data.info.Companyname || this.data.info.createman|| this.data.info.begintime|| this.data.info.endtime) {
      ...
      groupExpressSent(this.data.info).then(res => {
        if (res.code == 10000) {
          item = res.List;
          list = util.listData(item.reverse(), app.globalData.department, this.data.pages, list, this, 'ExpressSent');
          this.setData({
